...
interface ArticleRefreshButtonProps {
  slug: string;
}

export default function ArticleRefreshButton({ slug }: ArticleRefreshButtonProps) {
  ...
  return (
    <div className="flex gap-2">
      ...
      >
        {isRefreshing ? 'Обновление...' : 'Обновить статью'}
      </button>
      
      <button 
        onClick={() => window.location.href = `/articles/${slug}?v=${Date.now()}`}
